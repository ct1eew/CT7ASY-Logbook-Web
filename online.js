...
async function calculate(){const data=Object.fromEntries(new FormData($('propForm')));if(!locatorPosition(data.tx)||!locatorPosition(data.rx)){$('propStatus').textContent='Preencha locators TX e RX válidos.';return;}const current=++epoch;table=null;drawWheel();$('propCalculate').disabled=true;$('propStatus').textContent='A calcular…';try{const r=await api('/propagation',{...data,power:Number(data.power),rxantenna:data.txantenna});if(epoch!==current)return;table=r.table;drawWheel();$('propStatus').textContent='Previsão: '+r.date.split('-').reverse().join('-')+' · Horas UTC';}catch(e){if(epoch===current)$('propStatus').textContent=e.message;}finally{$('propCalculate').disabled=false;}}
function drawWheel(){
 const bands=[['3','80m'],['5','60m'],['7','40m'],['10','30m'],['14','20m'],['18','17m'],['21','15m'],['24','12m'],['28','10m']];const hour=new Date().getUTCHours(),band=bandFor($('propFrequency').value);let svg='<svg viewBox="0 0 500 500" role="group" aria-label="Previsão por banda e hora UTC">';
 const pt=(r,a)=>[250+r*Math.sin(a),250-r*Math.cos(a)].join(' ');
 for(let h=0;h<24;h++){const a=(h-.5)*Math.PI/12,b=(h+.5)*Math.PI/12;svg+=`<text x="${250+233*Math.sin(h*Math.PI/12)}" y="${254-233*Math.cos(h*Math.PI/12)}" text-anchor="middle" fill="#bbd4e5" font-size="11">${String(h).padStart(2,'0')}</text>`;bands.forEach(([key,label],i)=>{const inner=52+i*18,outer=inner+18,v=table?Number(table[h][key]):null,chosen=h===hour&&(!band||label===band),d=`M ${pt(inner,a)} L ${pt(outer,a)} A ${outer} ${outer} 0 0 1 ${pt(outer,b)} L ${pt(inner,b)} A ${inner} ${inner} 0 0 0 ${pt(inner,a)} Z`;svg+=`<path d="${d}" fill="${v===null?'#344859':`hsl(${(1-v)*180},95%,48%)`}" stroke="${chosen?'#fff':'#082235'}" stroke-width="${chosen?1.4:.7}" tabindex="0" role="button" data-hour="${h}" data-band="${label}" data-value="${v===null?'—':Math.round(v*100)+'%'}" aria-label="${h}:00 UTC, ${label}, ${v===null?'sem previsão':Math.round(v*100)+'%'}"/>`;});}
 svg+='<text id="wheelHour" x="250" y="240" text-anchor="middle" fill="white" font-size="13">Hora UTC</text><text id="wheelBand" x="250" y="259" text-anchor="middle" fill="white" font-size="13">Banda</text><text id="wheelValue" x="250" y="278" text-anchor="middle" fill="white" font-size="13">—</text>';
 ...
 const update=e=>{const p=e.target.closest('path[data-hour]');if(!p)return;$('wheelHour').textContent=p.dataset.hour.padStart(2,'0')+':00 UTC';$('wheelBand').textContent=p.dataset.band;$('wheelValue').textContent=p.dataset.value;};$('propWheel').onclick=update;$('propWheel').onfocusin=update;
}
